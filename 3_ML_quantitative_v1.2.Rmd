...
Group5 <- readRDS("chapter_3_data/Group5.rds")


Group5_GTA = read_csv("../3 eshot/Data/Group5_GTA_v3.csv")
...
G5_female_GTA18m_PRA <- read_xlsx("../3 eshot/PRA_Results/v3/g5_female_gta18m_comb_classification.xlsx", sheet = "Sheet1", range = "A10:J38")  %>% 
  left_join(gta_female_g5_18m_FS, 
            by = c("Dataset" = "feature_subset")) %>% 
  select(Dataset, n, NB, SMO, IBk, J48, mean, median)


G5_male_GTA18m_train_PRA <- 
  read_xlsx("../3 eshot/PRA_Results/v3/g5_male_gta18m_train_classification.xlsx", sheet = "Sheet1", range = "A10:J38") %>% 
  left_join(gta_male_g5_18m_FS, 
            by = c("Dataset" = "feature_subset")) %>% 
  select(Dataset, n, NB, SMO, IBk, J48, mean, median)

G5_zsex_ftp_train_PRA <- 
  read_xlsx("../3 eshot/PRA_Results/v3/g5_zsex_consol_ftp_classification.xlsx", sheet = "Sheet1", range = "A10:J36") %>% 
  left_join(ftp_zsex_g5_FS, 
            by = c("Dataset" = "feature_subset")) %>% 
  select(Dataset, n, NB, SMO, IBk, J48, mean, median)



GTA_Survival <- readr::read_rds("chapter_3_data/GTA_Survival.rds")


eshot_vars <- 
  read_xlsx("../3 eshot/Design/eshot_variables.xlsx", sheet = "Sheet1", na = "NA") %>%
...
> Add appendix/figure as example of overlap and also comprehensive list of which variables were asked to each group.
>
...
>
> * Will try to make the bit expalining the 2s, 3s, and 4s more clear and also RFE versions - this is important and will be useful for the reader in Study 3 too.
...
> * Would a general method bit be useful for this chapter and then explain the deviations from this in each study? - leave seperate
...
This study is therefore presented in three sections, one for each of the classification problems above.
...
A separate sample of Mountain Leader candidates were used for this study. We contacted all candidates who attended their first Mountain Leader training course in 2017 or 2018, inviting them to participate in the study, 1030 candidates started the survey and `r length(na.omit(Group5$SelfEfficacyDiscrepancy1))` completed the survey (`r round((length(na.omit(Group5$SelfEfficacyDiscrepancy1))/2867)*100,2)`% response rate). Of the `r length(na.omit(Group5$SelfEfficacyDiscrepancy1))` responses, `r Group5 %>% filter(SexId == 2) %>% .$SelfEfficacyDiscrepancy1 %>% na.omit() %>% length()` were from female candidates ($M$~age~ = `r Group5demo %>% filter(SexId == 2) %>% .$survey_age_mean` ±`r Group5demo %>% filter(SexId == 2) %>% .$survey_age_sd` years) and `r Group5 %>% filter(SexId == 1) %>% .$SelfEfficacyDiscrepancy1 %>% na.omit() %>% length()` were from male candidates ($M$~age~ = `r Group5demo %>% filter(SexId == 1) %>% .$survey_age_mean` ±`r Group5demo %>% filter(SexId == 1) %>% .$survey_age_sd` years). These candidates had been trained by `r Group5$firstMLTrainingProviderId %>% unique() %>% length()` different providers and those who had been assessed, had been assessed by `r Group5$firstMLAssessmentProvId %>% unique() %>% length()` different providers.

In Study 2 we split candidates according to whether or not they had DLOG data, which meant that it was not possible to then split each of these groups by gender as well because some groups would have been too small to analyse. However, in this study we only included DLOG data for the passing/not analyses where there does not seem to be a difference in outcomes by gender. This meant that we could analyse the data for male and female candidates separately for getting to assessment.

For the getting to assessment analyses, some candidates completed the survey retrospectively with regards to the criterion variable (i.e., they completed the survey at least 18 months after their training course). Others completed the survey prior to this, although some of them had been assessed and would therefore have answered some of the questions in a retrospective manner. The remaining candidates completed the survey in a truly prospective fashion (i.e., they had been trained less than 18 months before completing the survey and had not been assessed). In an ideal world, we would analyse the retrospective data to develop a model and then test it on the prospective data once a sufficient time period had passed (i.e., all of the candidates had been trained at least 18 months before analysing their data). See Table \@ref(tab:g5-candidate-survey-time) for a summary. Each of the analyses had different inclusion criteria and subsequently, used a different subset of candidates. Details of the candidates included in each analysis are presented below.
...
bind_rows(
  Group5_GTA %>%
    ...
    mutate(
      ...
      Sex = case_when(SexId == 2 ~ "Female",
                      SexId == 1 ~ "Male"),
      Time = case_when(
        TrainToSurvey >= 1.5 ~ "Retrospective",
        TrainToSurvey < 1.5 ~ "Prospective"
        ),
        Outcome = case_when(Assessed_18months == 0 ~ "Not assessed",
                            Assessed_18months == 1 ~ "Assessed")
    ) %>%
    group_by(Analysis,
             Sex,
             Time,
  ...
  Group5_FTP %>%
    ...
    mutate(
      ...
      Sex = case_when(SexId == 2 ~ "Female",
                      SexId == 1 ~ "Male"),
      Time = case_when(
        is.na(AssessChangeStandard) == TRUE ~ "Prospective",
        is.na(AssessChangeStandard) == FALSE ~
          "Retrospective"
      ),
        Outcome = case_when(FirstTimePass == 0 ~ "Did not pass",
                            FirstTimePass == 1 ~ "Passed")
    ) %>%
    group_by(Analysis,
             Sex,
             Time,
  ...
  pivot_wider(
    id_cols = c(Analysis, Sex, Outcome),
    names_from = Time,
    values_from = n
  ) %>% 
  kable(caption = "Candidates.", booktab = TRUE, 
        format = if (is_latex_output()) {"latex"} else {"html"}) %>%
  collapse_rows(valign = "top", columns = c(1:2)) %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                  full_width = FALSE) %>% 
    add_footnote(label = "Note: GTA = Getting to assessment within 18 months, FTP = First time pass",
                 notation = "none")
```
...
There were 65 responses from male candidates who completed the survey more than 18 months after their training course (i.e., retrospectively), 33 of whom had been assessed within 18 months of their training course and 32 who not been assessed at the time of completing the survey^[Candidates who had not been assessed within 18 months of their training course but had been assessed prior to completing the survey were excluded from the analysis as the wording of the questions shown to them meant they would not be comparable to the other candidates.]. Therefore, we were able to create a set of *training data* ($n$ = `r nrow(G5_male_GTA_18m_train)`), which we could use to select variables and a set of *test data* ($n$ = `r nrow(G5_male_GTA_18m_test)`, with an equal split of candidates who had and had not been assessed). 

In addition to this, `r nrow(G5_male_GTA_18m_pred) - 1` male candidates completed the survey more than 12 months after their training but less than 18 months after their training (i.e., prospectively). Using the model developed with the training data, we made predictions for each of these candidates which we have been able to test as all of them are now more than 18 months post training. 

> Is this truly prospective? What about the candidates who had been assessed when they completed the survey?
> * how many in this have already been assessed? 
> * get new data and do a genuinally prospective group
> * call it a "validation group" not truely prospective 
...
G5_female_GTA_18m_comb %>% 
  left_join(select(Group5, c(CandidateId, TrainToSurvey)), 
            by = "CandidateId") %>% 
...
plot_ly(
    ...
    link = list(
      ...
      value =  c(
        # female and male
        nrow(filter(na.omit(Group5_GTA), SexId == 2)),
        nrow(filter(na.omit(Group5_GTA), SexId == 1)),

        # split by time since training
        nrow(filter(na.omit(Group5_GTA), SexId == 2 & TrainToSurvey >= 1.5)),
        nrow(filter(na.omit(Group5_GTA), SexId == 2 & between(TrainToSurvey, 1, 1.5))),
        nrow(filter(na.omit(Group5_GTA), SexId == 2 & TrainToSurvey < 1)),
        nrow(filter(na.omit(Group5_GTA), SexId == 1 & TrainToSurvey >= 1.5)),
        nrow(filter(na.omit(Group5_GTA), SexId == 1 & between(TrainToSurvey, 1, 1.5))),
        nrow(filter(na.omit(Group5_GTA), SexId == 1 & TrainToSurvey < 1)),
        ...
        # male analysis groups
        10, # test
        nrow(filter(na.omit(Group5_GTA), SexId == 1 & TrainToSurvey >= 1.5)) - 10, # train
        nrow(filter(na.omit(Group5_GTA), SexId == 1 & between(TrainToSurvey, 1, 1.5))), # prospective
        
        # # not assessed
        # nrow(filter(Group5, SexId == 2 & TrainToSurvey < 1 & Assessed == 0)), # 0-12m female
        # nrow(filter(Group5, SexId == 1 & TrainToSurvey < 1 & Assessed == 0)), # 0-12m male
        # nrow(filter(Group5, SexId == 2 & TrainToSurvey > 1 & Assessed == 0)), # combined
        # 5, # test ** this isn't quite right - some went on to be assessed**
        # nrow(filter(Group5, SexId == 1 & TrainToSurvey >= 1.5 & Assessed == 0)), # train
        # nrow(filter(Group5, SexId == 1 & between(TrainToSurvey, 1, 1.5) & Assessed == 0)), # prospective
        
        # assessed
        nrow(filter(na.omit(Group5_GTA), SexId == 2 & TrainToSurvey < 1 & Assessed == 1)), # 0-12m female
        nrow(filter(na.omit(Group5_GTA), SexId == 1 & TrainToSurvey < 1 & Assessed == 1)), # 0-12m male
        nrow(filter(na.omit(Group5_GTA), SexId == 2 & TrainToSurvey >= 1 & Assessed == 1)), # combined
        5, # test ** this isn't quite right - some went on to be assessed**
        nrow(filter(na.omit(Group5_GTA), SexId == 1 & TrainToSurvey >= 1.5 & Assessed == 1)), # train
        nrow(filter(na.omit(Group5_GTA), SexId == 1 & between(TrainToSurvey, 1, 1.5) & Assessed == 1)), # prospective
        
        # outcome
        nrow(filter(Group5, FirstTimePass == 1 & is.na(SelfEfficacyDiscrepancy1) == FALSE)), # passed
        nrow(filter(Group5, FirstTimePass == 0 & is.na(SelfEfficacyDiscrepancy1) == FALSE)) # did not pass
...
* The measures identified in Study 2 were used (see **LINK**)
...
The most substantive change to the method used in Study 2, was the way that decentralisation for feature selection was carried out. For this study, features were vertically distributed [i.e., split by features and not by case; @Bolon-Canedo2015]. For each classification problem we created the following seven disjoint feature subsets (for a list of variables in each subset see Appendix **REF**):

* Psychosocial (n = 33)
* Training (n = 35)
* Consolidation (n = 57)
* Assessment (for FTP only)
* DLOG experience prior to training (n = 68; DLOG t)
* DLOG experience 6 months post training (DLOG t6; for GTA only)
* DLOG experience 12 months post training (DLOG t12; for GTA only)
* DLOG experience 18 months post training (DLOG t18; for GTA only)
* DLOG experience prior to assessment (DLOG a; for FTP only)
* Previous Mountain Training courses (for FTP only)

We then carried out the feature section process as described in Study 2 on each of the feature subsets above as well as the full set of features, resulting in at least two feature subsets, one for variables selected by at least two FS algorithms and one for variables selected by at least three FS algorithms, for each of the feature (sub-)sets. Where there were at least five variables selected by all four FS algorithms we also retained that feature subset. We then created three sets of omnibus feature subsets for each classification problem; an example of the omnibus features subsets is presented below for getting to assessment:
...
These analyses were carried out using LOO-CV.

* Selected top 20 as long as merit > 0 for those that used a Ranker search and then selected all from FCBF
* Leave one out cross validation (LOO-CV)
* k = 6 for Relief-F and IBk
* Not keeping a hold-out test data set can increase the estimate of classification accuracy [@Kuncheva2018]
* Decentralised feature selection [@Bolon-Canedo2015a; @Bolon-Canedo2015]
  * Vertically distributed [i.e., by features and not by case; @Bolon-Canedo2015].
  * Partitioned into the following disjoint feature subsets (for a list of variables in each subset see Appendix **REF**):
   
  * Following omnibus feature subsets were then created:
    * Survey data
    * DLOG data
    * Survey and DLOG data
  * We also created a model by including all features in the first instance ("all in").
* Created **??** models in total, from these we selected three as the "best" models. It is important to recognise that these three solutions are not the only useful ones, however, they were the models that bets classified the training data. It is also important to note that we considered the classification profile for each model, rather than just the mean score. It is not uncommon for one classifier (often J48) to perform much worse than the others, therefore if a model performed well with three classifiers and poorly with another that model was preferred to one that performed better on average (i.e., had a greater mean classification accuracy). For example, consider the classification profiles of the following models: Model A - NB = 85, SMO = 90, IBk = 85, J48 = 50 (mean = 77.5) and Model B - NB = 80, SMO = 80, IBk = 80, J48 = 80 (mean = 80). In this example we would prefer Model A to Model B.
* Training data and then hold out test data
  * Preferred way of analysing the data:
    1. Randomly select x candidates for each category of the criterion variable to be held-out as a test data set $D_{test}$.
    2. Prepare both the $D_{train}$ and $D_{test}$ data sets separately (e.g., standardising the data).
    3. With the remaining candidates $D_{train}$, perform the feature selection process.
  ...
  * As the data in $D_{test}$ has not been "seen" by the model during the feature selection stage, the classification rate does not suffer from the optimistic bias introduced through "peeking" [@Kuncheva2018; @Reunanen2003a; @Smialowski2010].
...
> * Should I consider the consistency of prediction between algorithms and come up with a single prediction for each instance and then test the accuracy of that? How many cases are incorrectly classified by all four algorithms? How many are only misclassified by one algorithm? etc.
> * Should I present the full tables for each analysis in this section (e.g., Table \@ref(tab:g5-PRA-all-models-classification-rates)? They could go into supplementary information/an appendix and I can just keep Table \@ref(tab:g5-PRA-best-models-classification-rates).
>    * Add the number of features for each model to each table.

#### Getting to assessment within 18 months of training

##### Male candidates

The classificaiton rates for both the training and test data were very good (i.e., approx 90%), however the predictive classification rates were lower (approx 70%). This suggests that the model developed is not overfitted to the training data, as there is little difference in the training and test classification rates. there are (at least) two plausible reasons for the lower classification rate in the predictive data. Firstly, it is possible that candidates are creating narritives in their minds based on whether or not they have been assessed and that had they answered the survey before being assessed, then their answers may have been different. A second explination is that because candidates were not at the same point in time when they answered the survey, their answers were different to those they would have given six-months later. For example, a candidate may not have felt that they had made much progress towards becoming a Mountian Leader 6-12 months post-training and then may make progress in the 12-18 month period.


###### Model selection

We tested `r nrow(G5_male_GTA18m_train_PRA)` models using the training data; the performance of each model can be seen in Table \@ref(tab:g5-male-gta18m-train-models). There were three models tied with the best perfomance, these models were made up of similar variables.
...
```{r male-GTA-18m-plot-omnibus-survey-3s-3s, fig.cap="male-GTA-18m-plot-omnibus-survey-3s-3s", cache=FALSE}
gta_male_g5_18m_FS %>% 
...
```{r male-GTA-18m-plot-omnibus-2s-3s, fig.cap="male-GTA-18m-plot-omnibus-2s-3s", cache=FALSE}
gta_male_g5_18m_FS %>% 
...
G5_zsex_ftp_train_PRA %>% 
  mutate(Rating = case_when(between(median, 0, 60) ~ "Poor",
    ...
    between(median, 90, 100) ~ "Excellent")) %>% 
  select(-c(mean, median)) %>% 
  kable(caption = "Group 5 passing first time, training model performance", digits = 2, booktab = TRUE) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
```
...
As with the results for [Male candidates](#males), **how important becoming a Mountain Leader is to a candidate, relative to other life goals**, is an important discriminatory variable for female candidates. We would expect this variable to have the same implications as those already discussed for male candidates. Again, the more **progress that candidates have made towards becoming a Mountain Leader**, the more likely they are to feel that they have **prepared effectively for a Mountain Leader assessment** and in doing so, they will have gained experience that boosts their confidence in their abilities to perform task related to the assessment. It is likely that **professional change** will have similar effects for female candidates as social change does for male candidates.
...
Whilst there may be a subset of candidates that are not represented in the data collected as part of this project, we believe that the findings presented in this report can be used to make a positive impact on the completion rate of the Mountain Leader qualification. This belief is based not only on the analyses of retrospective and prospective data presented here, but their congruence with the results from the [initial qualitative study](#Study1) and existing literature.
